 ...
 * Versão: 100% Estabilizada | Filtros | Drawer | Sincronizado
 */
...
// 2. Carregar Lista de Projetos para o primeiro Dropdown (Troca de Projeto)
...
// 3. Carregar Status cadastrados para este projeto
...
// 5. Lista de Meses para filtro (Extraído dinamicamente do banco)
...
function calcularSituacaoTarefa($t, $hoje, $lista_status) {
    ...
    if (stripos($nome_status, 'Concluído') !== false || stripos($nome_status, 'Concluido') !== false) {
        $concl = $t['data_conclusao'] ?? $hoje;
        return ($concl <= $t['data_fim']) 
            ? '<span class="badge bg-success shadow-sm" style="font-size:10px;">ENTREGUE NO PRAZO</span>' 
            : '<span class="badge bg-warning text-dark shadow-sm" style="font-size:10px;">ENTREGUE COM ATRASO</span>';
    }
...
<div class="sidebar-mini shadow no-print">
    <a href="../portal.php" title="Workspace"><i class="fas fa-th-large fa-2x"></i></a>
...
<div class="main-wrapper">
    ...
    <nav class="nav-board d-flex justify-content-between align-items-center shadow-sm">
        ...
        <div class="d-flex gap-2">
            <button class="btn btn-outline-dark btn-sm rounded-pill px-3 fw-bold" data-bs-toggle="modal" data-bs-target="#modalStatus">STATUS</button>
    ...
    <div class="filter-section no-print shadow-sm">
        <form method="GET" action="quadro.php" class="d-flex flex-wrap gap-2 align-items-center w-100">
            ...
            <select class="form-select form-select-sm" style="width:180px;" onchange="window.location.href='quadro.php?id='+this.value">
                <?php foreach($meus_projects as $mp) { ?>
                    <option value="<?php echo $mp['id']; ?>" <?php echo ($id_quadro == $mp['id']) ? 'selected' : ''; ?>>Projeto: <?php echo htmlspecialchars($mp['nome']); ?></option>
                <?php } ?>
            </select>

            <select name="f_status" class="form-select form-select-sm" style="width:160px;">
                <option value="">Status: Todos</option>
                <?php foreach($lista_status as $st) { ?>
                    <option value="<?php echo $st['id']; ?>" <?php echo ($f_status == $st['id']) ? 'selected' : ''; ?>><?php echo $st['label']; ?></option>
                <?php } ?>
            </select>

            <select name="f_mes" class="form-select form-select-sm" style="width:140px;">
                <option value="">Mês: Todos</option>
                <?php foreach($lista_meses as $m) { ?>
                    <option value="<?php echo $m; ?>" <?php echo ($f_mes == $m) ? 'selected' : ''; ?>>Mês: <?php echo date('m/Y', strtotime($m . "-01")); ?></option>
                <?php } ?>
            </select>

            <div class="d-flex align-items-center gap-1 border rounded px-2 bg-light">
                <small class="fw-bold text-muted" style="font-size: 10px;">DE:</small>
                <input type="date" name="f_data_ini" class="form-control form-control-sm border-0 bg-transparent" value="<?php echo $f_data_ini; ?>" style="width:130px;">
                <small class="fw-bold text-muted" style="font-size: 10px;">ATÉ:</small>
                <input type="date" name="f_data_fim" class="form-control form-control-sm border-0 bg-transparent" value="<?php echo $f_data_fim; ?>" style="width:130px;">
            </div>

            <button type="submit" class="btn btn-sm btn-dark px-3 rounded-pill fw-bold">FILTRAR</button>
            <?php if($f_status || $f_mes || ($f_data_ini && $f_data_fim)) { ?>
                ...
                <table class="table-clean">
                    ...
                    <tbody>
                        ...
                        <tr class="task-row">
                            ...
                            <td class="text-center"><button class="btn btn-sm btn-light border rounded-pill px-3 fw-bold" onclick="abrirPainelDetalhes(<?php echo $t['id']; ?>, '<?php echo addslashes($t['titulo']); ?>')">Abrir</button></td>
                        </tr>
                        <?php } ?>
...
<div class="offcanvas offcanvas-end shadow-lg" tabindex="-1" id="painelTarefa">
    ...
    <div class="offcanvas-body p-4 bg-light">
        <div class="card border-0 shadow-sm p-4 mb-4 rounded-4">
            <div class="row g-3">
                <div class="col-6"><label class="small fw-bold text-muted uppercase">Início</label><input type="date" id="p_inicio" class="form-control" onchange="salvarDetalhePainel('data_inicio', this.value)"></div>
                <div class="col-6"><label class="small fw-bold text-muted uppercase">Prazo</label><input type="date" id="p_fim" class="form-control" onchange="salvarDetalhePainel('data_fim', this.value)"></div>
                <div class="col-12 mt-3"><label class="small fw-bold text-muted uppercase">Justificativa / Comentário</label><textarea id="p_justificativa" class="form-control" rows="2" onblur="salvarDetalhePainel('justificativa', this.value)"></textarea></div>
                <div class="col-12 text-end mt-2"><button class="btn btn-sm btn-success px-4 rounded-pill" onclick="location.reload()">SALVAR PRAZOS</button></div>
            </div>
        </div>
        <div class="card border-0 shadow-sm p-4 rounded-4">
            <h6 class="fw-bold text-muted mb-3 small text-uppercase">Timeline / Prints</h6>
            <div id="editor-timeline" contenteditable="true" placeholder="Escreva ou cole um print (Ctrl+V)..."></div>
...
<div class="modal fade" id="modalStatus" tabindex="-1"><div class="modal-dialog modal-dialog-centered"><div class="modal-content p-4 border-0 shadow-lg">
    <h5 class="fw-bold mb-4">Gerenciar Status</h5>
    <div class="input-group mb-4"><input type="text" id="ns_label" class="form-control" placeholder="Novo..."><input type="color" id="ns_color" class="form-control form-control-color" value="#1a73e8"><button class="btn btn-primary" onclick="adicionarNovoStatus()">ADD</button></div>
...
<div class="modal fade" id="modalNovoGrupo" tabindex="-1"><div class="modal-dialog modal-dialog-centered"><form action="acoes.php" method="POST" class="modal-content shadow border-0"><div class="modal-body p-4">
    <input type="hidden" name="acao" value="novo_grupo"><input type="hidden" name="quadro_id" value="<?php echo $id_quadro; ?>"><label class="small fw-bold">NOME GRUPO</label><input type="text" name="nome_grupo" class="form-control mb-3" required><label class="small fw-bold">COR</label><input type="color" name="cor" class="form-control form-control-color w-100" value="#1a73e8"><button type="submit" class="btn btn-primary w-100 rounded-pill mt-4 fw-bold shadow">CRIAR</button>
</div></form></div></div>

<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
<script>
let curId = 0; 
const drawer = new bootstrap.Offcanvas(document.getElementById('painelTarefa'));
...
function salvarUpdateTimeline() {
    ...
    fetch('acoes.php', { method:'POST', body:fd }).then(() => { ed.innerHTML = ""; carregarTimeline(); });
}
