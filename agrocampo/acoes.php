...
try {
    ...
    if (isset($_GET['del_fin'])) {
        $pdo->prepare("DELETE FROM agro_financeiro WHERE id = ? AND usuario_id = ?")->execute([(int)$_GET['del_fin'], $uid]);
        header("Location: financeiro.php?sucesso=1"); exit;
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($acao === 'salvar_permissoes_agro') {
            ...
            header("Location: admin_permissoes.php?uid=$alvo&sucesso=1"); exit;
        }
    }
